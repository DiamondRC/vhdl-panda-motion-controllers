#!/usr/bin/env python3
"""
Generate lqr.block.ini + lqr_block.vhd for a chosen LQR/MPC controller shape.

Two ideas:
1. Derived sizes are computed with this script so the ini/wrapper and the
    engine never drift out of sync.
2. The per-axis structure (#POS/SP inputs, #U outputs) is emitted with loops,
    the rest is fixed scaffolding.

Setpoint demands scale by "orders": order 0 = position (SP), 1 = velocity (SV),
2 = acceleration (SA)... Each order is one AXES-wide slice of the REF block, so
adding an order is a generic bump, not a rewrite.

Export: when enabled the block masters ACP, exposing the write bundle as the
acp_mosi_t/acp_miso_t records (state_abi) so one port threads up per wrapper level.

We use jinja in PandABocks - Python 3.6 here for Diamond RedHat8 Machiones.

This file is largely AI generated!
"""

import argparse
import os


class Config:
    def __init__(self, axes=3, m=3, div=12500, hist_depth=2, scale=0.256, g_phi=0,
                 g_velocity=True, g_prev=True, g_uprev=True,
                 g_setpoint=True, sp_orders=2, g_affine=True,
                 g_export=True, base_addr="3FFF0000", chunk=8, dma=False):
        self.axes = axes
        self.m = m
        self.div = div
        self.hist_depth = hist_depth
        self.scale = scale
        self.g_phi = g_phi
        self.g_velocity = g_velocity
        self.g_prev = g_prev
        self.g_uprev = g_uprev
        self.g_setpoint = g_setpoint
        self.sp_orders = sp_orders    # per-axis setpoint demand orders (pos, vel, ...)
        self.g_affine = g_affine
        self.g_export = g_export       # master ACP with the exported state snapshot?
        self.base_addr = base_addr     # L2 window base (hex string, keep in sync w/ PandA)
        self.chunk = chunk             # export burst chunk (beats)
        self.dma = dma  # tier 1 (DMA long table) vs tier 2 (table short)


# --- setpoint demand orders: name + per per-axis slice -----------

SP_ORDER_NAMES = ["SP", "SV", "SA", "SJ"]
SP_ORDER_DESC = ["position", "velocity", "acceleration", "jerk"]


def sp_prefix(o):
    return SP_ORDER_NAMES[o] if o < len(SP_ORDER_NAMES) else "S%d" % o


def sp_desc(o):
    return SP_ORDER_DESC[o] if o < len(SP_ORDER_DESC) else "order-%d" % o


# --- mirror the VHDL sizing functions (cond_consts.vhd / lqr_consts.vhd) ---

def n_channels(g_state, g_velocity, g_prev):
    return int(g_state) + int(g_velocity) + int(g_prev)


def cond_width(p, g_state, g_velocity, g_prev):
    return p * n_channels(g_state, g_velocity, g_prev)


def n_int(n, m, ref, feats, uprev, setpoint, affine):
    # Mirrors lqr_consts.n_int(N, M, ref, feats, uprev, setpoint, affine).
    return n + feats + m * int(uprev) + ref * int(setpoint) + int(affine)


def resolve_ref(n, g_ref):
    # Mirrors lqr_consts.resolve_ref: 0 => default to N.
    return g_ref if g_ref > 0 else n


def state_width(c):
    # N: the conditioned state block width.
    return cond_width(c.axes, True, c.g_velocity, c.g_prev)


def ref_width(c):
    # Setpoint block width = orders AXES-wide demand slices (0 when unused).
    return c.sp_orders * c.axes if (c.g_setpoint and c.sp_orders > 0) else 0


def gain_count(c):
    n = state_width(c)
    ref = resolve_ref(n, ref_width(c))
    return c.m * n_int(n, c.m, ref, c.g_phi, c.g_uprev, c.g_setpoint, c.g_affine)


BANNER_INI = "# AUTOGENERATED -- configure with the Python generator, not this file"
BANNER_VHD = "-- AUTOGENERATED -- configure with the Python generator, not this file.\n"


def vbool(b):
    return "true" if b else "false"


def ent(c):
    # Distinct entity per tier so the two variants can coexist in one library
    # and a TB always binds the block it means to (no port-mismatch surprises).
    return "lqr_block_dma" if c.dma else "lqr_block"


def config_slug(c):
    # Fingerprint of the shape -> one output dir per config,
    # so different shapes can never overwrite each other's block.
    parts = ["a%d" % c.axes, "m%d" % c.m]
    if c.g_phi:
        parts.append("phi%d" % c.g_phi)
    if c.g_velocity:
        parts.append("vel")
    if c.g_prev:
        parts.append("prev")
    if c.g_uprev:
        parts.append("uprev")
    if c.g_setpoint:
        parts.append("sp%d" % c.sp_orders)
    if c.g_affine:
        parts.append("aff")
    if c.g_export:
        parts.append("exp")
    if c.div != 12500:
        parts.append("div%d" % c.div)
    if abs(c.scale - 0.256) > 1e-9:
        parts.append("s" + ("%g" % c.scale).replace(".", "p"))
    return "_".join(parts)


def make_block_ini(c):
    out = [BANNER_INI, "", "[.]",
           "description: LQR / nonlinear state-feedback motion controller.",
           "entity: %s" % ent(c)]
    if c.dma:
        out.append("type: dma")  # switches on the DMA-master wrapper section
    out += ["",
           "[INIT]", "type: bit_mux",
           "description: Reset / re-initialise the controller."]

    for a in range(c.axes):                                   # state inputs
        out += ["", "[POS%d]" % a, "type: pos_mux",
                "description: Axis %d interferometer position (state input)." % a]

    if c.g_setpoint:                                          # setpoint demands
        for o in range(c.sp_orders):
            for a in range(c.axes):
                out += ["", "[%s%d]" % (sp_prefix(o), a), "type: pos_mux",
                        "description: Axis %d %s setpoint." % (a, sp_desc(o))]

    if c.dma:                                                # tier 1: DMA long table
        out += ["", "[GAINS]", "type: table",
                "description: Flattened K matrix (row-major, Q7.25), "
                "DMAed into the inactive gain bank.",
                "31:0 K int", "    One Q7.25 gain word."]
    else:                                                    # tier 2: register-burst
        out += ["", "[GAINS]", "type: table short", "words: 1",
                "lines: %d" % gain_count(c),
                "description: Flattened K matrix (row-major, Q7.25), "
                "streamed into the inactive gain bank.",
                "31:0 K int", "    One Q7.25 gain word."]

    out += ["", "[COMMIT]", "type: write action", "wstb: true",
            "description: Swap the staged gains into the active bank ",
            "", "[GEN]", "type: read",
            "description: Gain generation counter -- read to confirm a swap has landed."]

    for k in range(c.m):                                     # control outputs
        out += ["", "[U%d]" % k, "type: pos_out",
                "description: Axis %d control output to DAC." % k]

    out += ["", "[U_VALID]", "type: bit_out",
            "description: DAC latch strobe -- one pulse per completed control pass.", ""]
    return "\n".join(out)


def make_block_vhd(c):
    out = ["-" * 80,
        "--  File:   %s.vhd" % ent(c),
        "--  Desc:   PandABlocks framework adapter around lqr_top.",
        "--  Author: richard.cunningham@diamond.ac.uk",
        "-" * 80, "",
        BANNER_VHD, "",
        "library ieee;",
        "use ieee.std_logic_1164.all;",
        "use ieee.numeric_std.all;", "",
        "use work.matrix_consts.all;",
        "use work.mac_utils.all;",
        "use work.num_utils.all;",
        "use work.lqr_consts.all;",
        "use work.cond_consts.all;",
        "use work.state_abi.all;", "", "",
        "entity %s is" % ent(c),
        "    port (",
        "        clk_i : in std_logic; -- PandA master clock",
        "        init_i : in std_logic; -- PandA Reset", ""]

    for a in range(c.axes):
        out.append("        pos%d_i : in std_logic_vector(31 downto 0); -- [POS%d] pos_mux" % (a, a))
    if c.g_setpoint:
        for o in range(c.sp_orders):
            for a in range(c.axes):
                out.append("        %s%d_i : in std_logic_vector(31 downto 0); -- [%s%d] pos_mux"
                           % (sp_prefix(o), a, sp_prefix(o), a))

    if c.dma:
        out += ["",
            "        -- Table (long): host writes ADDRESS + LENGTH, block masters the read",
            "        GAINS_ADDRESS : in std_logic_vector(31 downto 0);",
            "        GAINS_ADDRESS_WSTB : in std_logic;",
            "        GAINS_LENGTH : in std_logic_vector(31 downto 0); -- bytes",
            "        GAINS_LENGTH_WSTB : in std_logic;", "",
            "        -- DMA master bundle (wrapper binds to read engine)",
            "        dma_req_o : out std_logic;",
            "        dma_ack_i : in std_logic;",
            "        dma_done_i : in std_logic;",
            "        dma_addr_o : out std_logic_vector(31 downto 0);",
            "        dma_len_o : out std_logic_vector(7 downto 0); -- words",
            "        dma_data_i : in std_logic_vector(31 downto 0);",
            "        dma_valid_i : in std_logic;", ""]
    else:
        out += ["",
            "        -- Table short: words stream to DATA, no address on the bus",
            "        GAINS_START : in std_logic_vector(31 downto 0);",
            "        GAINS_START_WSTB : in std_logic;",
            "        GAINS_DATA : in std_logic_vector(31 downto 0);",
            "        GAINS_DATA_WSTB : in std_logic;",
            "        GAINS_LENGTH : in std_logic_vector(31 downto 0); -- informational",
            "        GAINS_LENGTH_WSTB : in std_logic;", ""]

    out += [
        "        -- Param bit + wstb: pulse swaps the staged bank (value unused)",
        "        COMMIT : in std_logic_vector(31 downto 0);",
        "        COMMIT_WSTB : in std_logic;", "",
        "        GEN : out std_logic_vector(31 downto 0); -- Read: generation tag", ""]

    for k in range(c.m):
        out.append("        u%d_o : out std_logic_vector(31 downto 0); -- [U%d] pos_out" % (k, k))

    out += ["",
        "        -- ACP write-master bundle (records thread one port per wrapper level)",
        "        m_axi_o : out acp_mosi_t;",
        "        m_axi_i : in acp_miso_t;", "",
        "        u_valid_o : out std_logic",
        "    );",
        "end entity;", "", "",
        "architecture main of %s is" % ent(c),
        "    -- Frozen build config",
        "    constant AXES : positive := %d;" % c.axes,
        "    constant M : positive := %d;" % c.m,
        "    constant DIV : positive := %d; -- 125MHz / DIV" % c.div,
        "    constant HIST_DEPTH : positive := %d;" % c.hist_depth,
        "    constant INTER_SCALE : real := %s; -- 256pm / count -> nm" % c.scale,
        "    constant G_PHI : natural := %d;" % c.g_phi,
        "    constant G_REF : natural := %d; -- %d orders x AXES" % (ref_width(c), c.sp_orders),
        "    constant G_VELOCITY : boolean := %s;" % vbool(c.g_velocity),
        "    constant G_PREV : boolean := %s;" % vbool(c.g_prev),
        "    constant G_UPREV : boolean := %s;" % vbool(c.g_uprev),
        "    constant G_SETPOINT : boolean := %s;" % vbool(c.g_setpoint),
        "    constant G_AFFINE : boolean := %s;" % vbool(c.g_affine),
        "    constant G_EXPORT : boolean := %s;" % vbool(c.g_export),
        "    constant BASE_ADDR : std_logic_vector(31 downto 0) := x\"%s\";" % c.base_addr,
        "    constant CHUNK : natural := %d;" % c.chunk, "",
        "    -- Derived",
        "    constant N : positive := cond_width(AXES, true, G_VELOCITY, G_PREV);",
        "    constant REF : positive := resolve_ref(N, G_REF);",
        "    constant GAIN_CNT : positive := M * n_int(N, M, REF, G_PHI, G_UPREV, G_SETPOINT, G_AFFINE);",
        "    constant GAIN_ADDR_W : positive := ceil_log2(GAIN_CNT);", "",
        "    -- Signals",
        "    signal pos_vec : mac_data_vec(0 to AXES - 1);",
        "    signal sp_vec : mac_data_vec(0 to REF - 1);",
        "    signal u_vec : lqr_out_vec(0 to M - 1);",
        "    signal gen_u : unsigned(GEN_W - 1 downto 0);", "",
        "    signal cnt : unsigned(GAIN_ADDR_W - 1 downto 0);",
        "    signal wr_addr : unsigned(GAIN_ADDR_W - 1 downto 0);",
        "    signal wr_data : signed(LANE_B_W - 1 downto 0);",
        "    signal wr_en : std_logic;", "",
        "    -- Gain-fill source seam",
        "    signal fill_start : std_logic;",
        "    signal fill_valid : std_logic;",
        "    signal fill_data : std_logic_vector(31 downto 0);", ""]

    if c.dma:
        out += [
        "    -- DMA request FSM",
        "    type dma_state is (IDLE, REQ, STREAM, DONE);",
        "    signal dma_fsm : dma_state;",
        "    signal words : unsigned(31 downto 0); -- remaining dwords to fetch",
        "    signal addr : unsigned(31 downto 0); -- byte address",
        "    signal blen : unsigned(8 downto 0); -- this burst's length (words)", ""]

    out += [
        "begin", "",
        "    -- State inputs: sign-extend pos bus (32b) -> lanes (42b)"]

    for a in range(c.axes):
        out.append("    pos_vec(%d) <= resize(signed(pos%d_i), LANE_A_W);" % (a, a))

    out += ["", "    -- Setpoint demands: [order0 | order1 | ...], each AXES-wide slice"]
    if c.g_setpoint:
        for o in range(c.sp_orders):
            for a in range(c.axes):
                out.append("    sp_vec(%d) <= resize(signed(%s%d_i), LANE_A_W);"
                           % (o * c.axes + a, sp_prefix(o), a))
    else:
        out.append("    sp_vec <= (others => (others => '0'));")

    out += ["", "    -- Control outputs: sign-extend DAC (7b) -> pos bus (32b)"]
    for k in range(c.m):
        out.append("    u%d_o <= std_logic_vector(resize(u_vec(%d), 32));" % (k, k))

    out += ["", "    -- Generation tag: zero-extend (16b) -> readback reg (32b)",
        "    GEN <= std_logic_vector(resize(gen_u, 32));", ""]

    if c.dma:
        out += [
        "    -- Gain-fill source: DMA stream (words land while STREAM)",
        "    dma_addr_o <= std_logic_vector(addr);",
        "    blen <= to_unsigned(256, blen'length) when words >= 256",
        "            else words(8 downto 0);",
        "    dma_len_o <= std_logic_vector(blen(7 downto 0));",
        "    fill_valid <= dma_valid_i when dma_fsm = STREAM else '0';",
        "    fill_data <= dma_data_i;", "",
        "    -- ",
        "    process(clk_i) begin",
        "        if rising_edge(clk_i) then",
        "            -- One-cycle pointer-reset pulse",
        "            fill_start <= '0';", "",
        "            if init_i = '1' then",
        "                dma_fsm <= IDLE;",
        "                dma_req_o <= '0';",
        "                words <= (others => '0');",
        "                addr <= (others => '0');",
        "            else",
        "                case dma_fsm is",
        "                    when IDLE =>",
        "                        dma_req_o <= '0';", "",
        "                        if GAINS_LENGTH_WSTB = '1' and unsigned(GAINS_LENGTH) >= 4 then",
        "                            words <= shift_right(unsigned(GAINS_LENGTH), 2); -- bytes -> dwords",
        "                            addr <= unsigned(GAINS_ADDRESS);",
        "                            fill_start <= '1'; -- reset the BRAM pointer for the load",
        "                            dma_fsm <= REQ;",
        "                        end if;", "",
        "                    when REQ =>",
        "                        dma_req_o <= '1'; -- addr + len already stable", "",
        "                        -- Fire the DMA stream!",
        "                        if dma_ack_i = '1' then",
        "                            dma_req_o <= '0';",
        "                            dma_fsm <= STREAM;",
        "                        end if;", "",
        "                    when STREAM =>",
        "                        -- Words flow to the seam, advance on burst done",
        "                        if dma_done_i = '1' then",
        "                            words <= words - blen;",
        "                            addr <= addr + shift_left(resize(blen, addr'length), 2);", "",
        "                            if words - blen = 0 then",
        "                                dma_fsm <= DONE;",
        "                            else",
        "                                dma_fsm <= REQ; -- next burst, pointer keeps counting",
        "                            end if;",
        "                        end if;", "",
        "                    when DONE =>",
        "                        dma_fsm <= IDLE; -- ready for the next load", "",
        "                end case;",
        "            end if;",
        "        end if;",
        "    end process;", ""]
    else:
        out += [
        "    -- Gain-fill source: register-burst stream",
        "    fill_start <= GAINS_START_WSTB;",
        "    fill_valid <= GAINS_DATA_WSTB;",
        "    fill_data <= GAINS_DATA;", ""]

    out += [
        "    -- Gain fill FSM: word-stream -> addressed BRAM write.",
        "    -- start resets the pointer, each valid word writes then increments.",
        "    process(clk_i) begin",
        "        if rising_edge(clk_i) then",
        "            wr_en <= '0'; -- strobe: default low, pulse per word", "",
        "            if init_i = '1' or fill_start = '1' then",
        "                cnt <= (others => '0');",
        "            elsif fill_valid = '1' then",
        "                wr_addr <= cnt;",
        "                wr_data <= signed(fill_data);",
        "                wr_en <= '1';",
        "                cnt <= cnt + 1;",
        "            end if;",
        "        end if;",
        "    end process;", "",
        "    -- The controller",
        "    u_lqr : entity work.lqr_top",
        "        generic map (",
        "            AXES => AXES,",
        "            M => M,",
        "            DIV => DIV,",
        "            HIST_DEPTH => HIST_DEPTH,",
        "            INTER_SCALE => INTER_SCALE,",
        "            G_PHI => G_PHI,",
        "            G_REF => G_REF,",
        "            G_EXPORT => G_EXPORT,",
        "            G_VELOCITY => G_VELOCITY,",
        "            G_PREV => G_PREV,",
        "            G_UPREV => G_UPREV,",
        "            G_SETPOINT => G_SETPOINT,",
        "            BASE_ADDR => BASE_ADDR,",
        "            CHUNK => CHUNK,",
        "            G_AFFINE => G_AFFINE",
        "        )",
        "        port map (",
        "            clk_i => clk_i,",
        "            init_i => init_i,",
        "            pos_i => pos_vec,",
        "            sp_i => sp_vec,",
        "            wr_addr_i => wr_addr,",
        "            wr_data_i => wr_data,",
        "            wr_en_i => wr_en,",
        "            commit_i => COMMIT_WSTB,",
        "            gen_o => gen_u,",
        "            u_o => u_vec,",
        "",
        "            m_axi_awvalid => m_axi_o.awvalid,",
        "            m_axi_awready => m_axi_i.awready,",
        "            m_axi_awaddr => m_axi_o.awaddr,",
        "            m_axi_awid => m_axi_o.awid,",
        "            m_axi_awlen => m_axi_o.awlen,",
        "            m_axi_awsize => m_axi_o.awsize,",
        "            m_axi_awburst => m_axi_o.awburst,",
        "            m_axi_awcache => m_axi_o.awcache,",
        "            m_axi_awuser => m_axi_o.awuser,",
        "            m_axi_awprot => m_axi_o.awprot,",
        "            m_axi_awlock => m_axi_o.awlock,",
        "            m_axi_awqos => m_axi_o.awqos,",
        "",
        "            m_axi_wvalid => m_axi_o.wvalid,",
        "            m_axi_wready => m_axi_i.wready,",
        "            m_axi_wid => m_axi_o.wid,",
        "            m_axi_wdata => m_axi_o.wdata,",
        "            m_axi_wstrb => m_axi_o.wstrb,",
        "            m_axi_wlast => m_axi_o.wlast,",
        "",
        "            m_axi_bvalid => m_axi_i.bvalid,",
        "            m_axi_bready => m_axi_o.bready,",
        "            m_axi_bresp => m_axi_i.bresp,",
        "            m_axi_bid => m_axi_i.bid,",
        "",
        "            u_valid_o => u_valid_o",
        "        );", "",
        "end architecture main;", ""]
    return "\n".join(out)


def target_dir(c):
    # One dir per slug
    root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    return os.path.join(root, "src", "lqr", config_slug(c))


def main():
    p = argparse.ArgumentParser(description="Generate the LQR PandA block for a given shape.")
    p.add_argument("--axes", type=int, default=3, help="number of state-axis inputs (POS*)")
    p.add_argument("--m", type=int, default=3, help="number of control outputs (U*)")
    p.add_argument("--div", type=int, default=12500, help="servo divider (125MHz / DIV)")
    p.add_argument("--scale", type=float, default=0.256, help="counts (256pm) -> nm scale")
    p.add_argument("--no-velocity", action="store_true", help="drop the velocity channel")
    p.add_argument("--no-prev", action="store_true", help="drop the previous-state block (x(k-1))")
    p.add_argument("--no-uprev", action="store_true", help="drop the u_prev feedback block")
    p.add_argument("--no-setpoint", action="store_true", help="drop the reference feedforward")
    p.add_argument("--sp-orders", type=int, default=2, help="per-axis setpoint demand orders (pos, vel, ...)")
    p.add_argument("--no-affine", action="store_true", help="drop the affine/bias column")
    p.add_argument("--no-export", action="store_true", help="drop the ACP state export")
    p.add_argument("--base-addr", type=str, default="3FFF0000", help="L2 export window base (hex)")
    p.add_argument("--chunk", type=int, default=8, help="export burst chunk (beats)")
    p.add_argument("--dma", action="store_true", help="tier 1: DMA long table (else table short)")
    p.add_argument("--stdout", action="store_true", help="print instead of writing the block files")
    args = p.parse_args()

    c = Config(axes=args.axes, m=args.m, div=args.div, scale=args.scale,
               g_velocity=not args.no_velocity, g_prev=not args.no_prev,
               g_uprev=not args.no_uprev, g_setpoint=not args.no_setpoint,
               sp_orders=args.sp_orders, g_affine=not args.no_affine,
               g_export=not args.no_export, base_addr=args.base_addr,
               chunk=args.chunk, dma=args.dma)

    ini, vhd = make_block_ini(c), make_block_vhd(c)
    print("# %d axes, %d outputs, N=%d, REF=%d -> %d gains (table lines=%d, wr_addr=%d bits)%s%s"
          % (c.axes, c.m, state_width(c), resolve_ref(state_width(c), ref_width(c)),
             gain_count(c), gain_count(c),
             max(1, (gain_count(c) - 1).bit_length()),
             ", setpoint x%d" % c.sp_orders if c.g_setpoint else "",
             ", export" if c.g_export else ""))

    ini_name = ("lqr_dma" if c.dma else "lqr") + ".block.ini"
    vhd_name = ent(c) + ".vhd"

    if args.stdout:
        print("\n===== %s =====\n" % ini_name + ini)
        print("\n===== %s =====\n" % vhd_name + vhd)
    else:
        out_dir = target_dir(c)
        os.makedirs(out_dir, exist_ok=True)
        with open(os.path.join(out_dir, ini_name), "w") as f:
            f.write(ini)
        with open(os.path.join(out_dir, vhd_name), "w") as f:
            f.write(vhd)
        print("# wrote %s + %s to %s" % (ini_name, vhd_name, out_dir))


if __name__ == "__main__":
    main()
